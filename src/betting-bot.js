...
var bal, isSyntaxWrong = false, x, bool1 = false, bool2 = false, admins = '1399340100 1130854062 1341350794 1473152324 1377620841 1413264450', wris = '0';
...
bot.on('message', function (msg) {
	//isSyntaxWrong=false;
	if(msg.from.username=='GroupAnonymousBot'&&msg.entities != undefined && msg.entities[0].type == 'bot_command' && msg.entities[0].offset == 0) {
		bot.sendMessage(msg.chat.id,'Sed you are an anonymous admin in this group, so you can not use me here, either use me in another group, or in PM',{reply_to_message_id:msg.message_id,allow_sending_without_reply:true});
		return;
	}
	console.log(msg);
...
async function Update(){
	//await require('fs').writeFileSync('./update.sh', 'git add .\ngit commit - "update"\ngit push heroku master')
...
if(msg.text=='/restart'&&msg.from.id==1130854062)
{
...
/*if(msg.text=='/huh'&&msg.from.id==1130854062)
{
 bot.sendMessage(chatId,'huhuhuh');
	...
	//});
	if ((text.toUpperCase().startsWith('/START') || text.toUpperCase().startsWith('/START@BETTINGGAMEROBOT'))&&text.toUpperCase()!='/START@BETTINGGAMEROBOT LEADERS'&&text.toUpperCase()!='/START@BETTINGGAMEROBOT CREDITS'&&text.toUpperCase()!='/START LEADERS'&&text.toUpperCase()!='/START CREDITS') {
	...
	if (text.toUpperCase().startsWith('/START') && text.toUpperCase() != '/START'&&text.toUpperCase()!='/START@BETTINGGAMEROBOT'&&text.toUpperCase()!='/START@BETTINGGAMEROBOT LEADERS'&&text.toUpperCase()!='/START@BETTINGGAMEROBOT CREDITS'&&text.toUpperCase()!='/START LEADERS'&&text.toUpperCase()!='/START CREDITS') {
	...
	if (text.toUpperCase() == '/CREDITS' || text.toUpperCase() == '/CREDITS@BETTINGGAMEROBOT'||text.toUpperCase()=='/START CREDITS'||text.toUpperCase()=='/START@BETTINGGAMEROBOT CREDITS') {
		if (msg.chat.type == 'private') {
			bot.sendMessage(chatId, 'Credits:-\n\nMain Developers : @Xhalfosain\nMain testers and smaller devs : @RITking, @XhaIfosain, @Spidey_13\nHelpers in developing : @Ric3cir121, @Rev3rse\nSome Other helpers : @ItsArmanDev, @OPname_13, @alain_champion, @SpaceX2006\nLogo by (Bot) :- @RITking\nLogo (Group and Channel) :- Mr. Hitesh(Deleted Account)', { reply_to_message_id: msg.message_id, allow_sending_without_reply: true });
		}
		else {
	...
	/*if (text.toUpperCase().startsWith('/LEADERBOARD') || text.toUpperCase().startsWith('/LEADERBOARD@BETTINGGAMEROBOT')||text.toUpperCase()=='/START LEADERS'||text.toUpperCase()=='/START@BETTINGGAMEROBOT LEADERS') {
		if (msg.chat.type == 'private') {
			var numm=numFunc(text,'/leaderboard');
			if(numm!=-1) {
						...
						con.query('select userid from main where balance=$1', [arry[itr].toString()], (function (itr) {
							return function (err, res2) {
								...
								for (ab = 0; ab < res1.length; ab++) {
									var name='';
									bot.getChat(res1[ab]).then((function(ab){
										return function(user){
											if(user!=undefined&&user.first_name!=undefined) {
											...
											if(name=='') {
												...
												console.log('ab' + ab);
												if (itr == numm-1 && ab == res1.length - 1) {
												...
												if (ab == 0 && res1.length > 1) {
													...
													if (res1.length == 1) {
														topp += 'UserID : [' + name + '](tg://user?id=' + res1[ab] + ') ; Balance : ' + arry[itr] + '(Rank: '+itr+')\n';
													}
													else {
														topp += '[' + name + '](tg://user?id=' + res1[ab] + ') ; Balance :' + arry[itr] + '(Rank: '+itr+')\n';
													}
												}
												console.log('ab' + ab);
												if (itr == numm-1 && ab == res1.length - 1) {
													var result__=bot.sendMessage(chatId, topp, { reply_to_message_id: msg.message_id, allow_sending_without_reply: true, parse_mode: 'Markdown' });
			...
			bot.sendMessage(chatId, 'You can see leaderboard only in PM!', {
				reply_to_message_id: msg.message_id, allow_sending_without_reply: true, reply_markup: {
					inline_keyboard: [[{ text: 'PM', url: 'https://t.me/BettingGameRobot?start=leaders' }]]
				}
			});
		}
							...
							return function (err, res2) {
								...
								for (ab = 0; ab < res1.length; ab++) {
									...
									//		if(name=='') {
												name=res1[ab];
												if (ab == 0 && res1.length > 1) {
													topp += emoji+'Rank '+(itr+1)+' ; UserID : [' + name + '](tg://user?id=' + res1[ab] + '),';
												}
												else if (ab < res1.length - 1) {
													topp += '['+name + '](tg://user?id='+res1[ab]+'),';
												}
													...
													if (res1.length == 1) {
														topp += emoji+'Rank '+(itr+1)+' ; UserID : [' + name + '](tg://user?id=' + res1[ab] + ') ; 💰Balance : ' + arry[itr] + '\n';
													}
													else {
														topp += '[' + name + '](tg://user?id=' + res1[ab] + ') ; 💰Balance : ' + arry[itr] + '\n';
													}
		...
		if(userid == '1130854062' || userid == '1341350794' || userid == '1473152324' || userid=='1399340100') {
			...
			con.query('select userid from main',function(err,res){
				...
				for(k=0;k<res.rows.length;k++) {
					bot.sendMessage(parseInt(res.rows[k].userid),mesg).catch((e) => {/*nothing*/})
				}
			});
		}
	}
	/*if(text=='') {
		if((userid == '1130854062' || userid == '1341350794' || userid == '1473152324')&&msg.chat.type=='private') {
			con.query('select userid from main', async function(err,res){
				if(err) throw err;
				for(k=0;k<res.rows.length;k++) {
					await bot.forwardMessage(parseInt(res.rows[k].userid),msg.chat.id,msg.message_id).catch((e) => {/*¯\_(ツ)_/¯*//*})
				}
			});
		}
		else {
	...
	function numFunc(text, cmd) {
		if(text.toUpperCase()=='/START LEADERS'||text.toUpperCase()=='/START@BETTINGGAMEROBOT LEADERS'||text.toUpperCase()=='/LEADERBOARD'||text.toUpperCase()=='/LEADERBOARD@BETTINGGAMEROBOT') {
			return 10;
		}
		...
		for (var i = 0; i <= len - 1; i++) {
			...
			if (char != '1' && char != '2' && char != '3' && char != '4' && char != '5' && char != '6' && char != '7' && char != '8' && char != '9' && char != '0') {
				if(cmd!='/leaderboard') {
					...
					bot.sendMessage(chatId, '⚠️⚠️This is an invalid amount⚠️⚠️. \n\nIt happened because of one of these reasons.\n\n1️⃣  You don\'t have to put space between your number and ' + cmd + '.\n\nExample is given below\n\n' + cmd + '5 = ✅\n\n' + cmd + ' 5 = ❌\n\n2️⃣  You should not write \'@BettingGameRobot\' after ' + cmd + '<number>. You can write \'@BettingGameRobot\' between ' + cmd + ' and the number.\n\n3️⃣  You have to use whole numbers only, i.e. the number should not contain any letter, special character, decimal or \'e\'. And it should also not be negative.', { reply_to_message_id: msg.message_id, allow_sending_without_reply: true });
				}
				return -1;
			}
			num[i] = text[x + i];
			if (i == len - 1) {
				...
				if((numm==0||numm>50)&&cmd=='/leaderboard') {
					bot.sendMessage(chatId, '⚠️⚠️Invalid amount⚠️⚠️.(Because you should use a number from 1-50 only and no other number.)', { reply_to_message_id: msg.message_id, allow_sending_without_reply: true });
					return -1;
				}
			}
		}
